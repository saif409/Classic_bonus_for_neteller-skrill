...
def index(request,page,subpage):
    if request.user.is_authenticated:
        ...
        if page == 'neteller':
            ...
            if subpage == 'deposits':
                subpage_is = 'deposits'
                neteller_deposits, neteller_counts,neteller_earnings, netellerID_groupBy,siteID_groupBy, id_gb_details \
            ...
            context = {}
        return render(request, 'sadmintemplates/stats/index.html', context)
...
def netellerDepositsFetch(request):
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST" and request.POST.get("netteller_filtering") == "neteller_filtered":
            ...
            if netidgroupby == "on":
                data = NetellerDeposit.objects.filter( Date__gte=datefrom,Date__lte=dateto).values('Neteller_ID').\
                ...
                # if request.method == "POST" and request.POST.get('gb_net_id_details') == 'neteller':
                #     netid = request.POST.get('gb_net_netid')
                #     id_gb_details = NetellerDeposit.objects.filter(Q(Neteller_ID__icontains=netid))
                #     print('gb_net_id_details', netid, id_gb_details)


            elif siteidGroupBy == "on":
                data = NetellerDeposit.objects.filter( Date__gte=datefrom,Date__lte=dateto).values('Site_ID').\
                    annotate(FinalCommssion=Sum('FinalCommssion'),TotalDeposite=Sum("Deposite")).values('Site_ID',
        ...
        return all_data, counts, earnings, netellerID_groupBy, siteID_groupBy, id_gb_details
    else:
        return redirect('admin-login')
...
def netellerSignupsFetch(request):
    if request.user.is_authenticated:
        ...
        if request.method == "POST" and request.POST.get("net_reg_submit") == "net_reg_submit_val":
            ...
            data = NetellerSignUp.objects.all().order_by("-Date")
...
def netellerApplications(request):
    if request.user.is_authenticated:
        ...
        if request.method == "POST" and request.POST.get("net_ap_submit") == "net_ap_submit_val":
            ...
            data = NetellerSignUp.objects.all()
